...
- **Keep-alive механизм**: автоматически поддерживает активность бота, предотвращая отключение на Replit и других платформах.
...
Бот автоматически поддерживает активность двумя способами:
1. **Периодические проверки**: каждые 5 минут бот проверяет своё состояние через Telegram API.
2. **HTTP health check**: если указана переменная окружения `PORT` (Replit предоставляет её автоматически), запускается простой HTTP сервер на этом порту. Можно настроить Replit на периодические запросы к `https://your-repl.repl.co/health` для дополнительной активности.

Бот не будет отключаться из-за бездействия на Replit.
